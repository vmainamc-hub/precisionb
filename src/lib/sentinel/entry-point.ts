...
import {
  simulateAllEntryDigits,
  simulateAllEntryDigitSessions,
  type EntryDigitSimResult,
  type BotEntrySessionReplayResult,
} from "../precision-edge/bot/simulator";
...
export interface EntryDigitScore {
  ...
  drivers: string[];
  cautions: string[];
  /** Exact legacy forced-contract replay evidence, retained for diagnostics. */
  dbotReplay?: EntryDigitSimResult;
  /** Real-bot session replay evidence: digit print -> actual 6-tick trigger -> runs. */
  sessionReplay?: BotEntrySessionReplayResult;
  /** Honest non-blank grade for the session evidence. */
  sessionGrade: "A" | "B" | "C" | "NO_MEASURED_EDGE";
  /** True when the real-bot session replay has enough sessions to measure. */
  sessionMeasured: boolean;
  /** Per-digit touch guard elimination; never invalidates the whole market×contract. */
  entryDigitTouchBlocked?: boolean;
...
export interface EntryPointReport {
  ...
  /** Explicit entry UX state; avoids collapsing blocked/no-edge/waiting into WAIT. */
  entryState: "MARKET_BLOCKED" | "NO_MEASURED_EDGE" | "DIGIT_READY_WAITING" | "ENTER_NOW";
  ...
  window: EntryWindow;
  invalidation: string[];
  /** Provenance of the preferred digit. Session-ranked is the new non-blank path. */
  selectionSource: "SESSION_RANKED" | "SIX_STAGE_DBOT_VALIDATED" | "NONE";
  ...
  /**
   * TOUCH GUARD — per-digit elimination, independent of point-scored factors.
   * A losing entry digit whose own pressure is BUILDING (dominant / emerging /
   * recovering) is removed from the entry candidate pool. The whole market×contract
   * is NOT invalidated; a safe runner-up can take its place.
   */
  /** Real-bot session replay audit used by the new entry selector. */
  sessionReplay: {
    enabled: boolean;
    candidateCount: number;
    measurableCount: number;
    preferredDigit: number | null;
    preferredGrade: EntryDigitScore["sessionGrade"] | "NONE";
  };
  /** Legacy forced-contract replay audit, retained for diagnostics/comparison. */
  dbotReplay: {
    enabled: boolean;
    candidateCount: number;
...
function sessionGradeOf(r: BotEntrySessionReplayResult | undefined): EntryDigitScore["sessionGrade"] {
  if (!r || r.measurableSessions < 12) return "NO_MEASURED_EDGE";
  const latencyOk = r.medianTriggerLatencyTicks !== null && r.medianTriggerLatencyTicks <= 18;
  if (r.firstRunWinRate >= 0.66 && r.fourWinRate >= 0.25 && r.ruinRate <= 0.15 && latencyOk) return "A";
  if (r.firstRunWinRate >= 0.60 && r.fourWinRate >= 0.18 && r.ruinRate <= 0.25) return "B";
  if (r.firstRunWinRate >= 0.52 && r.ruinRate <= 0.40) return "C";
  return "NO_MEASURED_EDGE";
}

function sessionRankingPoints(r: BotEntrySessionReplayResult | undefined): number {
  if (!r || r.measurableSessions < 8) return 0;
  const sampleAuthority = Math.min(1, r.measurableSessions / 40);
  const firstRun = (r.firstRunWinRate - 0.5) * 28;
  const fourWins = (r.fourWinRate - 0.15) * 18;
  const survival = Math.min(12, Math.max(-12, (r.medianRunsSurvived - 2.5) * 4));
  const ruin = Math.max(-12, -r.ruinRate * 24);
  const latency = r.medianTriggerLatencyTicks === null ? -4 : Math.max(-6, 5 - r.medianTriggerLatencyTicks * 0.25);
  return clamp((firstRun + fourWins + survival + ruin + latency) * sampleAuthority, -24, 24);
}
...
export function computeEntryPoint(inputs: EntryPointInputs): EntryPointReport {
  ...
  // Legacy forced-contract replay is retained as a diagnostic comparison only.
  // It is deliberately NOT a hard admission gate: the actual question is what
  // happens after the bot is activated by this digit and then follows its own
  // six-tick trigger.
  ...
  // NEW: real session replay. A historical print of X activates a virtual bot;
  // the bot then waits for its own trigger, executes only when that trigger
  // agrees with the candidate direction, and follows the real ladder/cadence.
  const sessionReplayResults: BotEntrySessionReplayResult[] = digits.length >= 40
    ? simulateAllEntryDigitSessions(digits, contract.side, {
        stopAfterRuns: 7,
        ladderDepth: 6,
        maxLosses: 6,
        maxSessionTicks: 120,
    ...
    : [];
  const sessionReplay = new Map<number, BotEntrySessionReplayResult>(
    sessionReplayResults.map((r) => [r.entryDigit, r]),
  );
  ...
  for (let d = 0; d < 10; d++) {
    ...
    // ── 13. REAL BOT SESSION REPLAY ────────────────────────────────────────
    const sr = sessionReplay.get(d);
    const sessionPts = Math.round(sessionRankingPoints(sr) * 10) / 10;
    const sessionGrade = sessionGradeOf(sr);
    if (sr) {
      factors.push({
        code: "SESSION_REPLAY",
        label: "Real-bot entry session replay",
        points: sessionPts,
        detail: `${sr.measurableSessions} historical activation sessions: first-run ${(sr.firstRunWinRate * 100).toFixed(1)}%, ≥4-win sessions ${(sr.fourWinRate * 100).toFixed(1)}%, median ${sr.medianRunsSurvived.toFixed(1)} runs survived, ruin ${(sr.ruinRate * 100).toFixed(1)}%, median trigger latency ${sr.medianTriggerLatencyTicks === null ? "n/a" : sr.medianTriggerLatencyTicks.toFixed(1)} ticks. Grade ${sessionGrade}.`,
      });
      if (sessionPts >= 3) drivers.push(`real-bot session replay supports this activation (${sessionGrade})`);
      if (sessionPts <= -3) cautions.push(`real-bot session replay is weak (${sessionGrade})`);
    } else {
      factors.push({
        code: "SESSION_REPLAY",
        label: "Real-bot entry session replay",
        points: 0,
        detail: "Insufficient history for session replay; this digit remains ranked, but no measured session edge is claimed.",
      });
    }
    ...
    scores.push({
      ...
      sessionReplay: sr,
      sessionGrade,
      sessionMeasured: !!sr && sr.measurableSessions >= 12,
      entryDigitTouchBlocked: false,
  ...
  const ordered = [...scores].sort((a, b) => b.score - a.score || b.pWinLower - a.pWinLower || a.digit - b.digit);

  // Touch guard is now PER DIGIT. Dangerous losing digits are removed from the
  // executable candidate pool, while every safe candidate remains eligible.
  // This prevents one bad digit from invalidating the entire market×contract.
  const BUILDING_LIFECYCLE = new Set(["dominant", "emerging", "recovering"]);
  const touchBlockedDigits = new Set<number>();
  for (const s of scores) {
    const lifecycle = pressure ? pressure.lifecycle[s.digit] : null;
    if (s.isLoser && !!lifecycle && BUILDING_LIFECYCLE.has(lifecycle)) {
      s.entryDigitTouchBlocked = true;
      touchBlockedDigits.add(s.digit);
    }
  }

  const selectable = ordered.filter((s) => !s.entryDigitTouchBlocked);
  let chosen = selectable[0] ?? ordered[0] ?? null;
  let changeState: EntryPointReport["changeState"] = "NEW";
  const held = sticky.get(key);
  if (chosen && held) {
    const heldNow = selectable.find((s) => s.digit === held.digit) ?? null;
    const regimeChanged = held.regime !== regimeLabel;
    const margin = heldNow ? chosen.score - heldNow.score : Infinity;
    if (heldNow && !regimeChanged && heldNow.digit !== chosen.digit && margin < MATERIAL_MARGIN) {
      chosen = heldNow;
      changeState = "HELD";
  ...
  const legacyValidated = ordered.filter((s) => dbotValidation.get(s.digit)?.accepted === true);

  const replayOrdered = selectable;
  const alternative = replayOrdered.find((s) => chosen && s.digit !== chosen.digit) ?? null;
  const runnerUp = alternative;
  const entryMargin = chosen && runnerUp ? Math.round((chosen.score - runnerUp.score) * 10) / 10 : 0;
  const sampleSize = scores.reduce((a, s) => a + s.n, 0);
  const lastDigit = intel.stats?.lastDigit ?? -1;

  // ── Status / per-digit touch guard ────────────────────────────────────
  ...
  const touchGuard = {
    ...
    reason: touchBlockedDigits.size
      ? `Per-digit touch guard removed losing digits ${[...touchBlockedDigits].join(", ")} because their pressure is currently BUILDING. The market×contract remains alive and falls back to the strongest safe entry digit.`
      : "No losing entry digit is currently BUILDING; no touch-guard elimination was required.",
  };
  ...
  const hasMeasuredSession = !!chosen && chosen.sessionMeasured;
  const readyGrade = chosen?.sessionGrade === "A" || chosen?.sessionGrade === "B";
  let status: EntryPointStatus;
  if (invalidated) status = "INVALIDATED";
  else if (!chosen) status = "NOT_READY";
  else if (readyGrade && chosen.digit === lastDigit) status = "ENTER NOW";
  else if (hasMeasuredSession) status = "ARMED";
  ...
  // ── Validity window: measured in bot sessions, not vague tick occurrences ─
  ...
  if (invalidated) {
    const psyBlockReason = inputs.canonicalPsychology?.contract?.hardBlock
      ...
      : null;
    const reason = danger.autoBlock.length > 0
    ...
    window = { kind: "UNVALIDATED", value: 0, label: "ENTRY WINDOW: BLOCKED", basis: reason };
  } else if (!chosen) {
    window = { kind: "UNVALIDATED", value: 0, label: "ENTRY WINDOW: NO SAFE DIGIT", basis: "All entry digits are currently eliminated by the per-digit touch guard." };
  } else if (!chosen.sessionMeasured) {
    window = {
      kind: "UNVALIDATED",
      value: 0,
      label: `ENTRY DIGIT ${chosen.digit}: NO MEASURED SESSION EDGE`,
      basis: `The system still ranks digit ${chosen.digit}, but only ${chosen.sessionReplay?.measurableSessions ?? 0} activation sessions are measurable; no false validation is claimed.`,
    };
  } else {
    const sessions = chosen.sessionReplay!.measurableSessions;
    const latency = chosen.sessionReplay!.medianTriggerLatencyTicks;
    window = {
      kind: "TICKS",
      value: Math.max(1, Math.round(latency ?? 1)),
      ticks: Math.max(1, Math.round(latency ?? 1)),
      label: status === "ENTER NOW"
        ? `ACTIVE ENTRY DIGIT · ${chosen.digit} · SESSION GRADE ${chosen.sessionGrade}`
        : `ARMED ON DIGIT ${chosen.digit} · SESSION GRADE ${chosen.sessionGrade}`,
      basis: `${sessions} measured activation sessions; first-run ${(chosen.sessionReplay!.firstRunWinRate * 100).toFixed(1)}%, median ${chosen.sessionReplay!.medianRunsSurvived.toFixed(1)} runs survived, ruin ${(chosen.sessionReplay!.ruinRate * 100).toFixed(1)}%, median trigger latency ${latency === null ? "n/a" : latency.toFixed(1)} ticks.`,
    };
  }
  ...
  const invalidation = [
    ...
    touchBlockedDigits.size
      ? `TOUCH GUARD: digits ${[...touchBlockedDigits].join(", ")} eliminated individually because losing-side pressure is BUILDING; safe fallback remains eligible`
      : "TOUCH GUARD: no entry digit currently requires elimination",
  ];

  const whyPreferred = chosen
    ? [
        `Entry digit ${chosen.digit} — score ${chosen.score.toFixed(0)}/100.`,
        ...(touchBlockedDigits.size ? [`⚠ ${touchGuard.reason}`] : []),
  ...
  const summary = chosen
    ? `${status} — preferred entry digit ${chosen.digit} for ${contract.label} on ${intel.name}; session grade ${chosen.sessionGrade}, confidence ${confidence}/100. ${window.label}.`
    : `No safe entry digit is currently available for ${contract.label} on ${intel.name}.`;
  ...
  return {
    ...
    window,
    invalidation,
    selectionSource: chosen ? "SESSION_RANKED" : "NONE",
    sampleSize,
    rankingDelta,
    changeState,
    ...
    sessionReplay: {
      enabled: sessionReplayResults.length > 0,
      candidateCount: sessionReplayResults.length,
      measurableCount: sessionReplayResults.reduce((n, r) => n + (r.measurableSessions >= 12 ? 1 : 0), 0),
      preferredDigit: chosen?.digit ?? null,
      preferredGrade: chosen?.sessionGrade ?? "NONE",
    },
    dbotReplay: {
      enabled: dbotReplayResults.length > 0,
      candidateCount: scores.length,
      validatedCount: legacyValidated.length,
