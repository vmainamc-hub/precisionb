...
export function hasValidatedEntryDigit(ep?: EntryPointReport | null): boolean {
  if (!ep || !ep.preferred) return false;
  const measured = ep.preferred.sessionGrade === "A" || ep.preferred.sessionGrade === "B";
  return measured && (ep.status === "ENTER NOW" || ep.status === "ARMED");
}
